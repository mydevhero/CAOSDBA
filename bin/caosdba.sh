...
purge_project() {
    ...
    echo "  - $CMAKE_SOURCE_DIR/queries/custom/ (all files)"
    echo "  - $CMAKE_SOURCE_DIR/schemas/custom/ (all files)"
    ...
    case $confirm in
        [yY])
            ...
            if [ -d "$CMAKE_SOURCE_DIR/queries/custom" ]; then
                rm -rf $CMAKE_SOURCE_DIR/queries/custom/*
                ls -al "$CMAKE_SOURCE_DIR/queries/custom/"
                echo "Removed $CMAKE_SOURCE_DIR/queries/custom/*"
            else
                echo "queries/custom/* directory not found, skipping"
            fi

            # Remove schemas directory (for JSON schema validation)
            if [ -d "$CMAKE_SOURCE_DIR/schemas/custom" ]; then
                rm -rf $CMAKE_SOURCE_DIR/schemas/custom/*
                ls -al "$CMAKE_SOURCE_DIR/schemas/custom/"
                echo "Removed $CMAKE_SOURCE_DIR/schemas/custom/*"
            else
                echo "schemas/custom/* directory not found, skipping"
            fi
...
init_project() {
    ...
    case $PROJECT_TYPE in
        ...
        CROWCPP)
            ...
            CROWCPP_TYPE=$(read_cmake_var "$CMAKE_SOURCE_DIR/cmake/crowcpp_type.cmake" "CROWCPP_TYPE")
            if [ $? -ne 0 ] || [ -z "$CROWCPP_TYPE" ]; then
                echo "Error: Could not read CROWCPP_TYPE from cmake/crowcpp_type.cmake"
    ...
    case $PROJECT_TYPE in
        ...
        BINDING)
            ...
            if [ -d "$CMAKE_SOURCE_DIR/queries/custom" ]; then
                echo "Custom query definitions directory created: $CMAKE_SOURCE_DIR/queries/custom"
                echo "  Note: Add custom queries to $CMAKE_SOURCE_DIR/queries/custom/queries.yaml"
            fi

            # Check if schemas/custom directory were copied
            if [ -d "$CMAKE_SOURCE_DIR/schemas/custom" ]; then
                echo "Custom schema directory created: $CMAKE_SOURCE_DIR/schema/custom"
                echo "  Note: Add custom schemas to $CMAKE_SOURCE_DIR/schemas/custom/queries.json"
            fi
            ;;
    ...
    if [ -d "$CMAKE_SOURCE_DIR/queries" ]; then
        echo "  Custom queries directory: $CMAKE_SOURCE_DIR/queries/custom"
        echo "  Custom schemas directory: $CMAKE_SOURCE_DIR/schemas/custom"
        echo ""
        echo "To add custom queries:"
        echo "  1. Edit $CMAKE_SOURCE_DIR/queries/custom/queries.yaml"
        echo "  2. Add corresponding schema to $CMAKE_SOURCE_DIR/schemas/custom/queries.json"
