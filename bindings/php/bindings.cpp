...
/**
 * Argument information for caos_hello()
...
#ifdef CAOS_PHP_DEBUG
  php_printf("CAOS: Installing INTERNAL execution hook\n");
#endif
  original_execute_internal = zend_execute_internal;
  zend_execute_internal = caos_execute_internal;
#ifdef CAOS_PHP_DEBUG
  php_printf("CAOS: Internal hook installed successfully\n");
#endif
...
zend_module_entry caos_module_entry =
{
  ...
  "caos",                     // Extension name
  caos_functions,             // Function list
  PHP_MINIT(caos),            // Module init
  PHP_MSHUTDOWN(caos),        // Module shutdown
  NULL,                       // Request init
  NULL,                       // Request shutdown
  PHP_MINFO(caos),            // phpinfo() callback
  "0.1",                      // Version
...
/**
 * caos_hello() - Simple test function without authentication
 *
 * Returns a greeting string. This function does not require authentication
 * and is not intercepted by the authentication hook.
 *
 * Usage: caos_hello()
 *
 * @return string "Hello from CAOS!\n"
 */
PHP_FUNCTION(caos_hello)
{
  (void)execute_data;
